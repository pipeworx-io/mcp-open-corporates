 ...
 *
 * NOT free/keyless any more, despite what this header used to claim.
 * OpenCorporates closed public v0.4 access: every unauthenticated call now
 * comes back 401. We shipped this pack against the old open tier and never
 * noticed the door shut, so the tools stayed advertised as working while
 * 100% of calls failed — reported by an oauth-tier user 2026-07-28.
 * There is no platform key (OpenCorporates is a paid, per-seat licence), so
 * the honest posture is BYO key + an error that says so.
 *
 * Tools:
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'OpenCorporates');
}

const BASE = 'https://api.opencorporates.com/v0.4';

/** OpenCorporates calls its credential `api_token`; the gateway convention is
 *  `_apiKey`. Accept either so a caller who read our docs and a caller who read
 *  theirs both work. */
function ocToken(args: Record<string, unknown>): string | undefined {
  for (const k of ['_apiKey', 'api_token', 'apiKey'] as const) {
    const v = args[k];
    if (typeof v === 'string' && v.trim()) return v.trim();
  }
  return undefined;
...
/** Turn an OpenCorporates HTTP failure into something a caller can act on.
 *  A bare "OpenCorporates API error: 401" reads as our outage; it isn't — it
 *  means no credential was supplied. The `_apiKey` token in the message is
 *  what makes the gateway classify this as auth_required rather than as our
 *  bug (see classifyToolError in workers/gateway/src/index.ts). */
function ocError(status: number, detail?: string): Error {
  if (status === 401 || status === 403) {
    return new Error(
      `OpenCorporates requires an API key — public access to the v0.4 API is closed, so unauthenticated calls return ${status}. Pass your own token via _apiKey (accounts at https://opencorporates.com/api_accounts/new). For keyless company lookup use the gleif pack instead: search_lei({name:"..."}) covers globally-registered legal entities with no key.`,
    );
  }
  return new Error(`OpenCorporates API error: ${status}${detail ? ` — ${detail}` : ''}`);
}
...
const tools: McpToolExport['tools'] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        _apiKey: { type: 'string', description: 'Your OpenCorporates API token (required — get one at https://opencorporates.com/api_accounts/new)' },
      },
      required: ['query'],
    },
  },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  const token = ocToken(args);
  switch (name) {
    case 'search_companies':
      return searchCompanies(args.query as string, args.country as string | undefined, token);
    case 'get_company':
      return getCompany(args.jurisdiction as string, args.number as string, token);
    case 'search_officers':
      return searchOfficers(args.query as string, token);
    default:
...
async function searchCompanies(query: string, country?: string, token?: string) {
  const params = new URLSearchParams({ q: query, format: 'json' });
  if (country) params.set('country_code', country);
  if (token) params.set('api_token', token);

  const res = await pwFetch(`${BASE}/companies/search?${params}`, {
    headers: { Accept: 'application/json' },
  });
  if (!res.ok) throw ocError(res.status);
...
async function getCompany(jurisdiction: string, number: string, token?: string) {
  const params = new URLSearchParams({ format: 'json' });
  if (token) params.set('api_token', token);

  const res = await pwFetch(`${BASE}/companies/${jurisdiction}/${number}?${params}`, {
    headers: { Accept: 'application/json' },
  });
  if (!res.ok) throw ocError(res.status, `company ${jurisdiction}/${number} not found`);

  const data = (await res.json()) as OCCompanyResponse;
  return formatCompany(data.results.company);
}

async function searchOfficers(query: string, token?: string) {
  const params = new URLSearchParams({ q: query, format: 'json' });
  if (token) params.set('api_token', token);

  const res = await pwFetch(`${BASE}/officers/search?${params}`, {
    headers: { Accept: 'application/json' },
  });
  if (!res.ok) throw ocError(res.status);
